...
Template.level16.rendered = function() {
  
}

Template.level16.events({
	/*
	render blockly code
	*/
 'click #vis':function() {
...
'click #save':function() {
	...
	Meteor.call('saveBlocks', {
		username: userName,
		ws: ws,
		level: 16,  // CHANGE THIS TO LEVEL NUMBER
	}, (err, res) => {
		if (err) {
			alert(err);
		} else {
...
'click #load':function() {
	...
	var handle = Meteor.subscribe("loadBlocks", userName, 16, {  // CHANGE THIS TO LEVEL NUMBER
			onError: function() {console.log(error)},
			onReady: function() {
				console.log('ready');
				const ws = UserBlocks.findOne({username: userName, level: 16}).ws;  // CHANGE THIS TO LEVEL NUMBER
