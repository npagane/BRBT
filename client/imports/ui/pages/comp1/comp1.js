...
Template.comp1.events({
  ...
  'click #save':function() {
    ...
    Meteor.call('saveBlocks', {
      username: userName,
      ws: ws,
      level: 100,  // CHANGE THIS TO LEVEL NUMBER
    }, (err, res) => {
      if (err) {
        alert(err);
      } else {
  ...
  'click #load':function() {
    ...
    var handle = Meteor.subscribe("loadBlocks", userName, 100, {  // CHANGE THIS TO LEVEL NUMBER
        onError: function() {console.log(error)},
        onReady: function() {
          console.log('ready');
          const ws = UserBlocks.findOne({username: userName, level: 100}).ws;  // CHANGE THIS TO LEVEL NUMBER
