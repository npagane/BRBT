...
Blockly.Blocks['procedures_callnoreturn'] = {
  ...
  setProcedureParameters_: function(paramNames, paramIds) {
    ...
    for (var i = 0; i < paramNames.length; i++) {
      try {
        if (this.agruments_[i] == paramNames[i]) {
          numEquals += 1;
        }
      } catch(error) {
        // indexing error, nothing in this.arguments_
      }
    }
    if (numEquals == paramNames.length) {
      // No change.
      this.quarkIds_ = paramIds;
      return;
    }
    // END CHANGE NICOLEPAGANE
